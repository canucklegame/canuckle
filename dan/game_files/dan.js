(this.canuckle = this.canuckle || {}),
  (this.canuckle.bundle = (function (e) {
    ...
    function saveArchive(gameApp) {
      ...
      window.localStorage.setItem("dan"+JSON.stringify(currentGame), JSON.stringify(saveData));
    }
    function saveArchiveNoApp() {
      var currentGame = Ga(new Date(JSON.parse(window.localStorage.getItem(archiveDate)))) - 235;
      var gameState = window.localStorage.getItem("dan-gameState");
            ...
            },
            {
              ...
              value: function () {
                ...
                this.shadowRoot.appendChild(Ka.content.cloneNode(!0)),
                  ...
                  this.sizeBoard(),
                  this.lastPlayedTs || window.localStorage.getItem("dan-hasPlayed") ||
                    setTimeout(function () {
                ...
                (this.shadowRoot.querySelector("#canuckle-title").textContent = 
                  "DAN'S DASH #".concat(Ga(new Date(JSON.parse(window.localStorage.getItem(archiveDate)))) - 235)),
          ...
          o(t, [
            {
              ...
              value: function () {
                ...
                var funFacts = [
                "<p>Ultra running is a running race that is longer than a marathon (a race that is approximately 42.2 kilometres or 26.2 miles). The most common ultra running races begin at 50 kilometres, or 31.069 miles, and can span up to 100 miles long. Ultra races can take place on roads, trails or tracks, and due to their recent popularity, more than 70,000 athletes compete in ultra races each year.<br><br>Gary Robbins was the closest Canadian ultra runner ever to finish the near impossible Barkley Marathons narrowly missing the cutoff time in 2017.</p>", /*1 ultra*/
                "<p>A relay race is a track-and-field sport consisting of a set number of stages (legs), usually four, each leg run by a different member of a team. The runner finishing one leg is usually required to pass on a baton to the next runner while both are running in a marked exchange zone.<br><br>Andre De Grasse, Brendon Rodney, Aaron Brown and Jerome Blake celebrated winning bronze in the 4x100m relay at Tokyo 2020 and after a long delay, one of the sprinters from Great Britain tested positive for a banned substance which enabled Team Canada to get upgraded to silver.</p>", /*2 baton*/
                "<p>Trail running is a sport-activity which combines running (and where there are steep gradients, hiking) that is run on any unpaved surface.<br><br>The Trans Canada Trail is a mixed series of trails and is perfect for the avid outdoors person for not just running, but for biking & walking as well. It is Canada’s national trail and is the longest network of multi-use recreational trails in the world. Crossing Canada on the Trans Canada Trail by foot would take over two years, spanning a whopping 24,134 km!</p>", /*3 trail*/
                "<p>Early running shoes were scrappy, uncomfortable, and unattractive by today’s standards. The first shoes specifically designed for running were developed in 1852 by the Spalding Company. They had six spikes in the soles with low-cut kangaroo leather uppers. They were extremely expensive at the time and typically used by high-profile athletes only.<br><br>Since the 1950s, running shoes have varied greatly in construction, style, and features. It has become an extremely competitive market with many niches.</p>", /*4 shoes*/
                "<p>Surge is the act of picking up the pace/speed during a race. Andre De Grasse is a Canadian sprinter whose race style starts off explosive - you can see in his races how he surges to reach the finish line in amazing speed.<br><br>A six-time Olympic medallist, De Grasse is the reigning Olympic champion in the 200m, and also won the silver in the 200m in 2016. He won a second silver in the 4x100m relay in 2020.<br><br><strong>Thanks for playing Dan's Dash Canuckle Game!</strong></p>", /*5 surge*/
